...
@Autonomous
public class CraterFinalAutoOp extends LinearOpMode {
...
//    private DcMotor extension;
    ...
    private static int SelectionDirection = 2; //Default to middle
    @Override
...
//        fingers = hardwareMap.get(CRServo.class, "fingers");
    ...
    public void hitCenter(){
        ...
        encoderDrive(DRIVE_SPEED, 11.5, -11.5, 8.0);
    }
    public void hitLeft(){
        if (!opModeIsActive()) return;
        ...
        encoderDrive(DRIVE_SPEED/2, 6, -6, 5.0);
    ...
    public void hitRight(){
        ...
        encoderDrive(DRIVE_SPEED/2, -6, 6, 5.0);
        //turn towards depot
        encoderDrive(DRIVE_SPEED*turningConstant, 4, 4, 5.0);
        if (!opModeIsActive()) return;
        runtime.reset();
        //Moving at right angle
        encoderDrive(DRIVE_SPEED, 12.5, -12.5, 8.0);
    }
    public void RightBackToCrater() {
        //encoderDrive(DRIVE_SPEED*turningConstant, 0.5, 0.5, 20.0);
        encoderDrive(DRIVE_SPEED, -13.5, 13.5, 20.0);
        encoderDrive(DRIVE_SPEED*turningConstant, 0.5, 0.5, 20.0);
        encoderDrive(DRIVE_SPEED*2/3, -9.5, 9.5, 20.0);
        sleep(500);
        encoderDrive(DRIVE_SPEED/8, -2.5, 2.5, 20.0);
        telemetry.addData("Path", "Complete");
        telemetry.update();
    }
    public void LeftDriveBackToCrater() {
        //Driving to the depot
        encoderDrive(DRIVE_SPEED*turningConstant, -0.5, -0.5, 20.0);
        encoderDrive(DRIVE_SPEED, -13.5, 13.5, 20.0);
        encoderDrive(DRIVE_SPEED*turningConstant, 1, 1, 20.0);
        encoderDrive(DRIVE_SPEED*2/3, -8.5, 8.5, 20.0);
        sleep(500);
        encoderDrive(DRIVE_SPEED/8, -2.0, 2.0, 20.0);
        telemetry.addData("Path", "Complete");
        telemetry.update();
    }
    public void driveFromCratertoDepot() {
        ...
        if (!opModeIsActive()) return;
        runtime.reset();
        //Second left turn to point towards depot
        encoderDrive(DRIVE_SPEED*turningConstant, 2.25, 2.25, 5.0);
        if (!opModeIsActive()) return;
        //GO INTO DEPOT!!
        encoderDrive(DRIVE_SPEED, 5.25, -5.25, 5.0);
        encoderDrive(DRIVE_SPEED*turningConstant, 0.65, 0.65, 5.0);
        encoderDrive(DRIVE_SPEED, 9.25, -9.25, 5.0);
        encoderDrive(DRIVE_SPEED/4, 1.0, -1.0, 20.0);
        if (!opModeIsActive()) return;
        runtime.reset();
        //unlock the sweeper
    ...
    public void driveBackToCrater() {
        encoderDrive(DRIVE_SPEED*turningConstant, -0.5, -0.5, 20.0);
        encoderDrive(DRIVE_SPEED, -13.5, 13.5, 20.0);
        encoderDrive(DRIVE_SPEED*turningConstant, 0.7, 0.7, 20.0);
        encoderDrive(DRIVE_SPEED*2/3, -9, 9, 20.0);
        sleep(500);
        encoderDrive(DRIVE_SPEED*2/3, -1.0, 1.0, 5.0); //Inch back to retouch the crater wall, incase it rolled off.
        telemetry.addData("Path", "Complete");
        telemetry.update();
    }
}
